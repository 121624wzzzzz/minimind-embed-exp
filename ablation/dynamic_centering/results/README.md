...
正式结果使用稳定的语义路径，不在目录名中编码运行时间。目前包含两套模型/数据配置：
...
紧凑产物。Qwen3-0.6B × FineEdu20B 跨配置复验直接保存三 seed aggregate 和逐 seed 对比，
避免复制已有 S1/S3/S12 基线归档。具体运行日志和 checkpoint 仍只保存在被忽略的 `logs/`、
